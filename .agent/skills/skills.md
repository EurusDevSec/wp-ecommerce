...
*   **Bài học:** Tránh việc import "mù" (import blind) hình ảnh hoặc asset từ các link ngoài khi chưa kiểm tra kỹ. Việc import nhầm ảnh lỗi dẫn đến việc phải đổi link, xóa sản phẩm và chạy lại script nhiều lần.
*   **Quy tắc:**
...
*   **Quy tắc:**
    *   Khi cần cập nhật lẻ tẻ, viết câu lệnh WP-CLI hoặc đoạn code PHP cập nhật trực tiếp (`wp post update <id> ...` hoặc `wp eval "wp_update_post(...)"`) thay vì xóa đi và import lại toàn bộ.
